...
|---|---|---|---|---|---|
| **L1 Identity** | 4 identity classes, Ed25519+X25519, rotation, revocation | snp-identity: NodeIdentity + Capability (extracted R2.2) + NodeId + derive_node_id + verify_signed. snp-crypto: Ed25519+X25519 primitives. | PASS | Conformance vectors 03-identity: 7/7 pass. snp-node tests: 97 pass. | R2.2 complete. Older skeleton types (DeviceCert, NodeDescriptor) retained but unused. |
...
| **L4 Discovery** | Peer+capability advertisement, freshness | snp-discovery: skeleton (Beacon, DescriptorStore with todo!()). snp-node: discovery.rs + peer_directory.rs + topology.rs (complete). GatewayAdvertisement carries mode capability. Extraction blocked by GatewayAdvertisement coupling (lives in snp-node, used by discovery types). | PARTIAL (crate), PASS (logic) | snp-node integration tests: n210, n211, n213 pass. | DiscoveredNode/DiscoveryProvider in snp-node depend on GatewayAdvertisement (in snp-node). Extraction requires first extracting GatewayAdvertisement. |
| **L5 Mesh Sync** | Anti-entropy, store-carry-forward, bundle custody | snp-sync: types defined (SyncRequest, SyncResponse, Bundle, CustodyHop) but all methods todo!(). TS reference: complete (sync.ts, 2316 lines). No Rust runtime integration. | MISSING (Rust), PASS (TS) | TS sync.ts exports 30+ functions. | Not implemented in Rust; N3 bypasses this entirely. Types defined but methods unimplemented. |
