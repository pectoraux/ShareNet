...
| [0003](./0003-simplified-noise-ik.md) | Simplified Noise_IK handshake structure | 1 | 🟡 proposed (human review required) | The spec mandates Noise_IK (§7.2) and the threat model (§4.2) says "use a vetted library; do not hand-roll." This ADR documents the sandbox's *structural* simplification (signed-descriptor exchange + DH-derived keys, no post-handshake AEAD yet) and explicitly defers to a vetted Noise library for production. **NOT production-safe.** |
