...
/// Authorization for a relay's position in a circuit, derived from the
/// committed route. The relay uses this to verify its position before
/// installing forwarding state.
///
/// ## P0 #2 — Replaces unsigned position fields
///
/// Previously, `RelayHandshakeRequest` carried unsigned `predecessor_node_id`,
/// `successor_node_id`, and `role` fields. A malicious source could supply
/// arbitrary values, and the relay would install forwarding state from these
/// unsigned fields.
///
/// `CommittedHopAuthorization` is DERIVED from the `CommittedRoute` (whose
/// commitment hash is signed by the source inside the `CircuitHandshake`).
/// The relay verifies the binding: `authorization.commitment_hash ==
/// handshake.commitment_hash` AND `authorization.circuit_id ==
/// handshake.circuit_id`. A malicious source cannot substitute a different
/// authorization (e.g., with a tampered predecessor) without breaking this
/// binding — the relay detects the mismatch and refuses to install state.
///
...
#[derive(Debug, Clone)]
pub struct CommittedHopAuthorization {
...
/// Derive a `CommittedHopAuthorization` for every non-source hop in a
/// committed route.
///
...
pub fn derive_hop_authorizations(
    route: &CommittedRoute,
    handshake: &CircuitHandshake,
) -> Result<Vec<CommittedHopAuthorization>, DistributedCircuitError> {
    ...
    for (i, hop) in hops.iter().enumerate() {
        ...
        // a CommittedHopAuthorization (it does not install forwarding state
        // from a request).
        if i == 0 {
            continue;
        }
        ...
        authorizations.push(CommittedHopAuthorization {
...
///   via `commitment_hash`).
/// - The `CommittedHopAuthorization` (derived from the `CommittedRoute`)
///   that the relay uses to verify its position before installing forwarding
///   state (P0 #2).
///
/// ## Trust boundary
///
/// The `handshake` is cryptographically signed by the source. The
/// `authorization` is NOT independently signed — it is verified by the relay
/// via binding checks (`authorization.commitment_hash ==
/// handshake.commitment_hash` AND `authorization.circuit_id ==
/// handshake.circuit_id`). A malicious source cannot substitute a different
/// authorization (e.g., with a tampered predecessor) without breaking this
/// binding.
...
pub struct RelayHandshakeRequest {
    ...
    /// The relay's position authorization, derived from the committed route.
    /// Replaces the previous unsigned `predecessor_node_id` /
    /// `successor_node_id` / `role` / `relay_node_id` fields (P0 #2).
    pub authorization: CommittedHopAuthorization,
}
...
/// ## Pipeline
///
/// 1. Derive `CommittedHopAuthorization`s for every non-source hop from the
///    `CommittedRoute` + `CircuitHandshake` (P0 #2 — the relay verifies its
///    position from this authorization, not from unsigned fields).
/// 2. For each authorization:
///    a. Construct a `RelayHandshakeRequest` (handshake + authorization).
...
///    f. **P0 #1**: Compute `DH(ephemeral_x25519_secret,
...
/// 3. If ALL relays acknowledged successfully, construct `ActiveCircuit`.
///
...
pub fn establish_distributed_circuit(
    ...
    // P0 #2: derive authorizations from the committed route. These replace
    // the previous unsigned position fields on RelayHandshakeRequest.
    let authorizations = derive_hop_authorizations(route, handshake)?;
    ...
    for auth in &authorizations {
        ...
        // CommittedHopAuthorization derived from the committed route.
...
/// 2. **Binding check**: Verify `authorization.commitment_hash ==
///    handshake.commitment_hash` AND `authorization.circuit_id ==
///    handshake.circuit_id`. A malicious source cannot substitute a different
///    authorization (e.g., with a tampered predecessor) without breaking
///    this binding — the relay detects the mismatch and refuses to install
///    state.
/// 3. Verify the `CircuitHandshake` (signature + freshness + NodeId binding).
/// 4. Check replay (`CircuitAcceptanceStore`).
/// 5. Compute DH (relay's X25519 private key + source's ephemeral public key).
/// 6. Derive the forwarding key (HKDF — same as the source did).
/// 7. Create a signed `RelayHandshakeResponse` with the DH proof.
/// 8. Install `RelayForwardingState` from `request.authorization` (NOT from
///    unsigned fields — P0 #2).
/// 9. Record acceptance in `CircuitAcceptanceStore` (replay prevention).
///
/// # Parameters
///
/// - `request`: the handshake request from the source. Contains the signed
///   `CircuitHandshake` + the `CommittedHopAuthorization` (derived from the
///   committed route).
...
/// Returns `DistributedCircuitError::RelayResponseInvalid` if the handshake
/// is invalid, the authorization binding check fails, or the handshake is a
/// replay.
pub fn accept_relay_handshake(
    ...
    // 2. Binding check: the authorization's commitment_hash + circuit_id must
    //    match the handshake's. The handshake is signed by the source (which
    //    binds the source to this specific route). The authorization must be
    //    for the SAME circuit/route — otherwise the source is trying to
    //    substitute a different authorization (e.g., with a tampered
    //    predecessor) and the relay refuses to install state.
    ...
    // 6. Derive the forwarding key (same HKDF as the source did).
    ...
    // 7. Compute the DH proof: SHA-256(dh_secret).
    let dh_proof = sha256(&dh_secret);

    // 8. Create the signed response.
    ...
    // 9. Install forwarding state from the authorization (NOT from unsigned
    //    fields — P0 #2). The authorization was derived from the committed
    //    route, so predecessor/successor/role are cryptographically bound to
    //    the route the source signed.
    ...
    // 10. Record acceptance (replay prevention).
