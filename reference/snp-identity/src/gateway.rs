//! GatewayAdvertisement — signed gateway discovery record (L1 layer).
//!
//! R2.2 extraction: this module was moved verbatim from
//! `snp-node/src/node/gateway.rs` to establish snp-identity as the
//! authoritative owner of all signed identity-bearing types. The CBOR
//! encoding, signature preimage, and verification logic are byte-for-byte
//! identical to the pre-extraction implementation — no field names, types,
//! or canonical-CBOR shapes were changed.
//!
//! The dependency direction is:
//!   snp-identity (owns GatewayAdvertisement + VerifiedGatewayAdvertisement)
//!       ↓
...
use crate::{now_unix, Capability, IdentityError, IdentityResult, NodeIdentity};

/// Default advertisement lifetime: 1 hour.
///
/// R2.2: moved from `snp-node/src/node/mod.rs` so the type and its TTL live
/// in the same crate.
pub const ADVERTISEMENT_TTL_SECS: u64 = 3600;

// ─── Small CBOR helpers (private to this module) ─────────────────────────────
//
// These mirror the helpers that previously lived in `snp-node/src/node/mod.rs`.
// They construct CborValue variants; they do NOT change the canonical
// encoding (RFC 8949 §4.2.1 sorting is performed by `snp_cbor::encode`).
...
fn extract_uint(v: CborValue, field: &str) -> IdentityResult<u64> {
    match v {
        CborValue::UnsignedInt(n) => Ok(n),
        other => Err(IdentityError::Other(format!(
            "GatewayAdvertisement.{field} must be a CBOR uint; got {other:?}"
        ))),
    }
}

fn extract_text(v: CborValue, field: &str) -> IdentityResult<String> {
    match v {
...
fn extract_bstr_32(v: CborValue, field: &str) -> IdentityResult<[u8; 32]> {
    match v {
...
fn extract_bstr_64(v: CborValue, field: &str) -> IdentityResult<[u8; 64]> {
    match v {
        CborValue::ByteString(bytes) => {
            if bytes.len() != 64 {
                return Err(IdentityError::Other(format!(
...
fn extract_caps(v: CborValue, field: &str) -> IdentityResult<Vec<Capability>> {
    match v {
        CborValue::Array(items) => {
            ...
            for item in items {
                ...
                caps.push(Capability::from_str(&s).ok_or_else(|| {
                    IdentityError::Other(format!("unknown capability \"{s}\""))
                })?);
            }
            Ok(caps)
        }
...
impl GatewayAdvertisement {
    ...
    ///
    /// # Errors
    /// Returns [`IdentityError::Other`] if CBOR encoding fails.
    pub fn encode_cbor(&self) -> IdentityResult<Vec<u8>> {
        ...
        Ok(snp_cbor::encode(&CborValue::Map(entries))
            .map_err(|e| IdentityError::Other(format!("CBOR encode GatewayAdvertisement: {e}")))?)
    }
    ...
    ///
    /// # Errors
    /// Returns [`IdentityError::Other`] if the bytes are not a valid
    /// GatewayAdvertisement.
    pub fn decode_cbor(bytes: &[u8]) -> IdentityResult<Self> {
        let value = snp_cbor::decode(bytes)
            .map_err(|e| IdentityError::Other(format!("CBOR decode GatewayAdvertisement: {e}")))?;
        ...
        Ok(Self {
            node_id: node_id.ok_or_else(|| IdentityError::Other("nodeId missing".into()))?,
            public_key: public_key.ok_or_else(|| IdentityError::Other("publicKey missing".into()))?,
            circuit_x25519_pub: circuit_x25519_pub.unwrap_or([0u8; 32]),
            listen_addr: listen_addr.ok_or_else(|| IdentityError::Other("listenAddr missing".into()))?,
            discovery_addr: discovery_addr
                .ok_or_else(|| IdentityError::Other("discoveryAddr missing".into()))?,
            capabilities: capabilities.unwrap_or_default(),
            egress_policy: egress_policy.unwrap_or_default(),
            timestamp: timestamp.ok_or_else(|| IdentityError::Other("timestamp missing".into()))?,
            expiry: expiry.ok_or_else(|| IdentityError::Other("expiry missing".into()))?,
            signature: signature.ok_or_else(|| IdentityError::Other("signature missing".into()))?,
            observed_rtt,
        })
    }
