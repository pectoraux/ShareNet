//! Node identity descriptors with verified/unverified distinction (L1 layer).
//!
//! R2.2 extraction: this module was moved verbatim from
//! `snp-node/src/node/descriptor.rs` to establish snp-identity as the
//! authoritative owner of all signed identity-bearing types. The CBOR
//! encoding, signature preimage, NodeId-consistency checks, and the
//! verified/unverified type-system distinction are byte-for-byte identical
...
///
/// [`VerifiedNodeAdvertisement`]: ../../snp_node/node/struct.VerifiedNodeAdvertisement.html
#[derive(Debug, Clone)]
...
impl VerifiedNodeDescriptor {
    /// Construct a `VerifiedNodeDescriptor` from the primitive fields of an
    /// already-verified advertisement.
    ///
    /// **R2.2 (DESCRIPTOR-EXTRACTION):** This is the renamed
    /// `from_verified_advert_internal` constructor — it was previously
    /// defined in `snp-node/src/node/descriptor.rs` and took a
    /// `&NodeAdvertisement` directly. Since `NodeAdvertisement` lives in
    /// `snp-node` (which depends on `snp-identity`), this constructor now
    /// takes the underlying primitive fields instead. The
    /// `VerifiedNodeAdvertisement::descriptor()` method in `snp-node` calls
    /// this constructor after extracting the primitive fields from its
    /// inner advert.
    ///
    /// # Panics
    /// Panics if `node_id != SHA-256("SNP/0.1 node\0" || ed25519_public_key)`.
    /// The caller MUST guarantee this (the upstream `verify_into_verified()`
    /// path checks it before calling).
    #[doc(hidden)]
    #[must_use]
    pub fn from_verified_advert_internal(
        node_id: [u8; 32],
        ed25519_public_key: [u8; 32],
...
pub struct VerifiedGatewayAdvertisement {
    pub(crate) advert: GatewayAdvertisement,
}
...
// ─── GatewayAdvertisement::verify_into_verified ─────────────────────────────
//
// R2.2: this `impl GatewayAdvertisement` block lives in `descriptor.rs`
// (alongside `VerifiedGatewayAdvertisement`) to mirror the pre-extraction
// layout where both were defined in `snp-node/src/node/descriptor.rs`. The
// construction accesses `VerifiedGatewayAdvertisement.advert`, which is
// `pub(crate)` so the cross-module construction works within `snp-identity`.
